...
import numpy as np
import torch
from torch_geometric.utils import is_undirected, to_undirected, to_scipy_sparse_matrix
...
def train(model, graph, loss_fn, optimizer):
    ...
    # Forward pass.
    pred = model.forward(graph.x, graph.edge_index, graph.distance)
    y = graph.y.squeeze()
...
def run(args, verbose: bool = True):
    ...
    # Dataset & model.
    dataset = get_dataset(
        root="./data/{}".format(args["dataset"]), name=args["dataset"]
    )
    ...
    for t in range(times):
        ...
        if verbose:
            print(
                "Training done. Test_acc (on best valid_acc = {:.2f}%) = {:.2f}%\n".format(
                    100 * best_val_acc, 100 * test_acc
                )
            )
